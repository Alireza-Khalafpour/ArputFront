...
const PulsePlan = () => {
  ...
  const [pulse_price_without_ar_id, set_pulse_price_without_ar_id] =
    useState("");
  ...
  async function GetPulsePrice(Au) {
    await axios
      .get(`${url}/pulse/price/admin/list`, {
        headers: {
          accept: "application/json",
          Authorization: `Bearer ${Au}`,
        },
  ...
  async function GetRowIdForActivate(id) {
    ...
    fetch(
      `${url}/pulse/plan/admin/active/${id}`,
      ActiveMethod
    )
      .then((response) => {
        response.json();
  ...
  const GetRowIdForDelete = (id) => {
    ...
    fetch(
      `${url}/pulse/plan/admin/deactive/${id}`,
      deleteMethod
    )
      .then((response) => {
        response.json();
  ...
  return (
    <div className="w-full">
      ...
      <Dialog
        ...
        maxWidth="sm"
        open={shopPlanModal}
        onClose={() => setShopPlanModal(false)}
      >
        <DialogTitle className="flex justify-center items-center rounded-xl w-full h-[3rem] bg-asliDark text-paszamine1">
        ...
        <DialogActions className="p-4 flex flex-row gap-4 mt-10">
          ...
          <Button
            variant="soft"
            color="danger"
            onClick={() => setShopPlanModal(false)}
          >
            انصراف
          </Button>
        </DialogActions>
