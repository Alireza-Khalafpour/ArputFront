...
export const CreateFactory = () => {
  ...
  const DeleteCategoryApi = (i) => {
    ...
    fetch(`${url}/category/delete`, deleteMethod)
      .then((response) => {
        response.json();
      })
      .then((d) => {
    ...
    renderRowActions: ({ row, table }) => {
      return (
        <Box sx={{ display: "flex", flexWrap: "nowrap", gap: "8px" }}>
          <IconButton color="secondary" onClick={() => GetRowIdForEdit(row)}>
            <EditRounded />
          </IconButton>
    ...
    renderDetailPanel: ({ row }) => (
      <div className="bg-paszamine1 p-2">
        <h3 className="w-full text-start font-extrabold mb-2">
          {" "}
          <span className="border-b-2 p-1 text-base"> ویژگی ها </span>{" "}
        </h3>
        <Box
          sx={{
            display: "grid",
          ...
          {row?.original?.features.map((item) => (
            <>
              ...
              <Typography className="flex flex-row gap-1 items-center">
                ...
                <p>وضعیت : </p>
                {item.active === true ? (
                  <p className="rounded-full p-2 bg-green-700 w-2 h-2"></p>
                ) : (
                  <p className="rounded-full p-2 bg-rose-700 w-2 h-2"></p>
                )}{" "}
              </Typography>
            </>
          ))}
