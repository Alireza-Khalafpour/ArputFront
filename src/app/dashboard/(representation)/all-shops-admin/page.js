...
const AllShopsAdmin = () => {
  ...
  async function GetRowIdToShop(id) {
    ...
    await axios
      .put(
        `${url}/register/admin/not-representative`,
        {
          shop_id: id,
        },
        {
  ...
  async function GetRowIdToBranch() {
    ...
    await axios
      .put(
        `${url}/register/admin/is-representative`,
        {
          shop_id: shopid,
          factory_id: facId,
        },
    ...
    await axios
      ...
      .then((response) => {
        if (response.data.Done === true) {
          setAlert(true);
          setMessage(
            `${response.data?.message} | موجودی جدید ${digitsEnToFa(
  ...
  const columns = useMemo(
    () => [
        ...
        Cell: ({ cell }) => (
          <span>
            {cell.getValue() === true ? (
              ...
              <span className="bg-red-500 text-white rounded-2xl p-1">
                {" "}
                غیرفعال{" "}
              </span>
            )}
      ...
      },
      {
        ...
        id: "is_branch",
        Cell: ({ cell }) => (
          <span>{cell.getValue() === true ? "نمایندگی" : "فروشگاه"}</span>
        ),
      },
    ],
    []
  ...
  return (
    <div className="w-full">
      ...
      <Dialog
        ...
        scroll="paper"
        maxWidth="sm"
        open={MakeBranchModal}
        onClose={() => setMakeBranchModal(false)}
      >
          ...
          <div className="flex flex-col justify-center items-center gap-10 w-full">
            <div className="w-full flex md:flex-row flex-col gap-7 justify-around items-center my-10 ">
              <Autocomplete
                ...
                sx={{ width: "190px" }}
                renderInput={(params) => (
                  <TextField {...params} variant="outlined" label=" کارخانه " />
                )}
              />
            </div>
          </div>
        ...
        <DialogActions className="p-4 flex flex-row gap-4 mt-10">
          ...
          <Button
            variant="soft"
            color="danger"
            onClick={() => setMakeBranchModal(false)}
          >
            انصراف
          </Button>
        </DialogActions>
