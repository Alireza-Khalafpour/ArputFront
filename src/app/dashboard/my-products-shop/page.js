...
export const MyProductsShop = () => {
  ...
  async function ListApi(Au) {
    await axios
      .get(`${url}/pre_product/shop/list/all`, {
        headers: {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-paszamine3 py-4">
        {" "}
        لیست پیش محصول ها | برای ایجاد محصول بر روی پیش محصول مورد نظر کلیک راست
        کنید یا دکمه " افزودن کالا " را بفشارید.{" "}
      </p>
      ...
      <ProuductsTableOfShopAndFactory
        triggerGetShopProductApi={triggerGetShopProductApi}
      />
              ...
              <div className="w-2/3 flex flex-col">
                <div className="flex flex-row justify-center items-end gap-5">
                  <div>
                    ...
                    >
                      {" "}
                      عکس های محصول را یک به یک آپلود کنید{" "}
                    </label>
                    <input
          ...
          <div className="flex flex-col mx-auto gap-4 w-full">
            ...
            <div className="grid grid-cols-2 gap-4 justify-around items-center w-full">
              {features.map((x) => (
                <p>
                  {" "}
                  {x.feature_name} : {x.feature_sample_name}{" "}
                </p>
              ))}
            </div>

            <h2 className="p-2 rounded-2xl bg-sky-200">
              {" "}
              واقعیت افزوده: {hasBundle == true ? "دارد" : "ندارد"}{" "}
            </h2>
          </div>
        </DialogContent>
      </Dialog>
