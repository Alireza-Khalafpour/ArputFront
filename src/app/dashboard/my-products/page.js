...
export const MyProducts = () => {
  ...
  async function GetRowIdForActivate(id) {
    ...
    fetch(
      `${url}/pre_product/admin/active?pre_product_id=${id}`,
      ActiveMethod
    )
      .then((response) => {
        response.json();
  ...
  const GetRowIdForDelete = (id) => {
    ...
    fetch(
      `${url}/pre_product/admin/deactive?pre_product_id=${id}`,
      deleteMethod
    )
      .then((response) => {
        response.json();
  ...
  async function ConfirmPreProduct(row) {
    ...
    fetch(
      `${url}/pre_product/admin/confirmation?pre_product_id=${row.original?.pre_product_id}`,
      confirmMethod
    )
      .then((response) => {
        response.json();
    ...
    renderRowActions: ({ row, table }) => {
      return (
        ...
        >
          ...
          {row.original?.admin_confirmation == true ? null : (
            <Button
              onClick={() => ConfirmPreProduct(row)}
              size="small"
              className="rounded-xl bg-khas hover:bg-orange-600 p-1 text-white font-semibold "
            >
              تایید
