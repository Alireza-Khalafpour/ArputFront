...
function PaymentWalletResponse() {
  ...
  async function PaymentFinish() {
    await axios
      .get(
        `${url}/payment/finished/?authority=${Authority}&status=${Status}`,
        {
          headers: {
    ...
    <div className="min-h-[80vh]">
      {response?.alert_stat == "success" ? (
        <div className="w-full flex flex-col justify-center items-center gap-6">
          ...
          />
          <h2 className="text-2xl ">{response?.stat_msg}</h2>
          <h3> شماره پیگیری : {response?.refid} </h3>
          <Link
            href="/dashboard/my-wallet"
            className="p-2 rounded-2xl bg-khas text-white"
          >
            {" "}
            بازگشت به کیف پول{" "}
          </Link>
        ...
        <div className="w-full flex flex-col justify-center items-center gap-6">
          ...
          <h2 className="text-2xl text-black">{response?.stat_msg}</h2>
          <Link
            href="/dashboard/my-wallet"
            className="p-2 rounded-2xl bg-khas text-white"
          >
