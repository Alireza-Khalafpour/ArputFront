...
const AddToFavoriteAndShare = ({pId}) => {
    ...
    async function AddToFavorites(pId) {
        await axios.post(`${url}/pre_product/favorite/create`, {
        "pre_product_id": `${pId}`
        }, 
        {
        headers: headers
    ...
    async function GetShopList(id) {
        ...
        setLoading(true)
        await axios.get(`${url}/product/product/${id}`).then((response) => {
    ...
    async function CreatePulse(i) {
        await axios.post(`${url}/pulse/create`,{
                                ...
                                <Card onClick={() => CreatePulse(i)} className="md:w-1/6 w-3/4 h-[200px] hover:shadow-2xl cursor-pointer" >
                                    ...
                                    <CardOverflow>
                                        ...
                                        <CardContent className="gap-3 m-auto text-right p-1 h-full w-full flex flex-col justify-center" >
                                            ...
                                            <div className="flex flex-row gap-1 items-center"> <Rating value={3} readOnly /> </div>
                                            <div className="p-1 text-black">
                                                {digitsEnToFa(addCommas(i.price))} ریال
                                            </div>
                                            <Typography className="w-full flex-nowrap text-nowrap !overflow-hidden" level="body-sm">
                                                {i.description}
                                            </Typography>
                                        </CardContent>
                                    </div>
