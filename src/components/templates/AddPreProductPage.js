...
const AddPreProductPage = () => {
  ...
  function handleSetTempFeature(i, index, val) {
    setTempFeatureSample({
      feature_id: i.id,
      sample_id: val.sample_data.sample_id,
    });
    setTempIndex(index);
  }
  ...
  const handleNextFeatureSample = () => {
    ...
    setTimeout(() => {
      setPreProductData({
        ...preProductData,
        ["features"]: preProductData.features.filter(
  ...
  const handleWidthChange = (event, newValue) => {
    ...
    setPreProductData({
      ...
      info: {
        width: newValue,
        height: preProductData.info.height,
        weight: preProductData.info.weight,
      },
    });
  };
  ...
  const handleHeightChange = (event, newValue) => {
    ...
    setPreProductData({
      ...preProductData,
      info: {
        width: preProductData.info.width,
        height: newValue,
        weight: preProductData.info.weight,
      },
    });
  ...
  async function handleImageUpload() {
    ...
    await axios
      ...
      .then((response) => {
        setPreProductData({
          ...preProductData,
          image_url: response?.data.address,
        });
        setLoading(false);
        setActiveStep((prevActiveStep) => prevActiveStep + 1);
      })
  ...
  async function GetCurrentUser() {
    await axios
      ...
      .then((response) => {
        setPreProductData({
          ...preProductData,
          factory_id: response?.data.data[0].id,
        });
      })
      .catch((error) => {
        console.log(error, "Error");
  ...
  const handleAddBranchIds = (val) => {
    setPreProductData({
      ...preProductData,
      only_in_Representation: val.map((i) => i.branch_id),
    });
  };
  ...
  return (
    <div className="w-full flex flex-col justify-center items-center gap-4">
      <h2 className="p-3 text-xl">
        {" "}
        | با انجام گام های زیر کالای خود را ثبت کنید |
      </h2>
        ...
        >
            ...
            <StepContent className="flex flex-col gap-12 p-3 m-5">
              <div className="flex flex-col justify-around items-center gap-6 w-full my-5">
                {addCateg?.features?.map((i, index) => (
                  <div
                    className={` flex flex-row justify-between mx-5 items-center p-3 md:w-[60%] w-full gap-4 ${
                      preProductData?.features[index]?.feature_id === i.id
                        ? "bg-green-200"
                        : "bg-paszamine1"
                    } rounded-lg border-asliLight border`}
                  >
                    ...
                    <Autocomplete
                      ...
                      options={sampleOptions.filter(
                        (s) => s.feature_data.id === i.id
                      )}
                      getOptionLabel={(option) =>
                        option.sample_data.feature_sample_name
                      }
                    ...
                    <div className="flex flex-row gap-4 w-1/4 justify-end items-center">
                      <button
                        className="bg-rose-600 text-white rounded-full hover:bg-rose-700 p-1"
                        onClick={() => deleteHandler(i, index)}
                      >
                        {" "}
                        <DeleteForeverRounded />{" "}
                      </button>
          ...
          <Step className="border-b-4 py-3">
            ...
            <StepContent className="flex flex-col gap-12 p-3 m-5">
              <div className="flex flex-row justify-around items-center gap-6 w-full my-5">
                <FormControlLabel
                  checked={preProductData.is_public}
                  onChange={() =>
                    setPreProductData({
                      ...preProductData,
                      is_public: !preProductData.is_public,
                      only_in_Representation: [],
                    })
                  }
                  control={<Checkbox defaultChecked />}
                  label=" قابل استفاده برای همه "
                />
