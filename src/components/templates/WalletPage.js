    ...
    renderBottomToolbar: ({ row }) => {
      return (
        <div className="w-full flex justify-end items-center flex-row gap-4 p-4 border-4">
          <span className="p-2 border border-khas text-asliLight font-semibold rounded-xl ">
            {" "}
            موجودی : {digitsEnToFa(addCommas(walletData?.balance))} ریال{" "}
          </span>
          <span className="p-2 border border-khas text-asliLight font-semibold rounded-xl ">
            {" "}
            مجموع کل واریزی ها : {e2p(0)}{" "}
          </span>
        </div>
      );
    },
